...
function Home(){
  ...
  return (
    ...
    <div className='w-full py-10 px-9 sm:px-10'>
        <div className={`max-w-5xl mx-auto grid ${cards.length === 1 ? 'place-items-center' : 'grid-cols-1 md:grid-cols-3 gap-8'}`}>
          {cards.length === 0 ? (
            <p className='text-2xl ml-[500px] mt-[200px] text-blue-800 '>NO POSTINGS TO DONATE</p>
          ) : (
            cards.map((card) => (
              <div className='shadow-lg rounded-lg overflow-hidden hover:scale-105 duration-300'>
                ...
                <div className='p-6'>
                  <h2 className='text-2xl font-bold mb-2 text-center'> {card.name}</h2>
